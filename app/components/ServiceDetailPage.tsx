    ...
    <main className="overflow-x-hidden">
      <section className="relative min-h-[62vh] overflow-hidden bg-[rgb(var(--color-primary-dark))] text-[rgb(var(--color-secondary-light))]">
        ...
        <div className="relative z-10 mx-auto flex min-h-[62vh] max-w-6xl flex-col justify-end px-4 pb-24 pt-14">
          <Link href="/services" className="soft-press mb-8 inline-flex w-fit items-center gap-2 rounded-full bg-white/12 px-4 py-2 text-sm font-semibold text-white backdrop-blur hover:bg-white/18">
            <ArrowLeft size={16} />
            Back to Services
          </Link>
      ...
      <section className="bg-[rgb(var(--color-cream))] py-14">
        <div className="mx-auto grid max-w-6xl grid-cols-2 gap-3 px-4 md:grid-cols-4">
          {gallery.map((src, i) => (
            <div key={src} className="media-zoom overflow-hidden rounded-2xl organic-ring">
      ...
      <section className="bg-[rgb(var(--color-secondary-light))] py-16">
        <div className="mx-auto grid max-w-6xl gap-8 px-4 lg:grid-cols-[1.15fr_0.85fr]">
          <div className="space-y-5">
            {sections.map((section) => (
              <article key={section.title} className="organic-card lift-card p-6 sm:p-8">
                ...
                {section.items && (
                  <ul className="mt-5 grid gap-3">
                    {section.items.map((item) => (
                      <li key={item} className="group flex gap-3 text-gray-700">
          ...
          <aside className="space-y-5">
            <div className="organic-card lift-card bg-white/95 p-6 sm:p-8">
              <h2 className="text-2xl font-bold text-[rgb(var(--color-ink))]">Best Fit For</h2>
              <div className="mt-5 space-y-3">
                {bestFor.map((item) => (
                  <div key={item} className="soft-press rounded-2xl bg-[rgba(var(--color-primary-light),0.5)] px-4 py-3 text-sm font-medium text-gray-800 hover:bg-[rgba(var(--color-primary-light),0.72)]">
                    {item}
                  </div>
                ))}
              </div>
            ...
            <div className="lift-card rounded-[2rem] bg-[rgb(var(--color-primary-dark))] p-7 text-[rgb(var(--color-secondary-light))] shadow-xl">
        ...
        <div className="mx-auto mt-10 grid max-w-6xl gap-5 px-4 lg:grid-cols-3">
          <article className="organic-card lift-card p-6">
          ...
          <article className="organic-card lift-card p-6">
            ...
            <div className="mt-4 grid gap-2 text-sm font-semibold text-[rgb(var(--color-primary))]">
              <Link href="/services/nutrition-meal-planning" className="inline-block hover:translate-x-1">Nutrition & Meal Planning</Link>
              <Link href="/services/workout-motivation-coaching" className="inline-block hover:translate-x-1">Workout & Motivation Coaching</Link>
              <Link href="/services/virtual-workshops" className="inline-block hover:translate-x-1">Virtual Wellness Workshops</Link>
            </div>
          </article>
        </div>
        ...
        {faqs.length > 0 && (
          <div className="mx-auto mt-10 max-w-4xl px-4">
            ...
            <div className="mt-5 space-y-3">
              {faqs.map((item) => (
                <details key={item.question} className="organic-card lift-card p-5">
